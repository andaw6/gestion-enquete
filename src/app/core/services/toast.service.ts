...
/**
 * Service responsable de l'affichage de notifications toast dynamiques dans le DOM.
 *
 * Les types supportés sont : `success`, `error`, `warning`, `info`.
 * Chaque toast est automatiquement supprimé après 5 secondes ou manuellement via le bouton de fermeture.
 *
 * @example
 * // Dans un composant :
 * constructor(private toast: ToastService) {}
 *
 * this.toast.show('Opération réussie !', 'success');
 * this.toast.show('Une erreur est survenue', 'error');
 */
@Injectable({
  providedIn: 'root',
})
export class ToastService {
  /** Permet de manipuler le DOM de manière sûre via Angular */
  ...
  /**
   * Affiche une notification dans le DOM avec un message et un type visuel.
   *
   * @param message - Le message à afficher.
   * @param type - Le type de toast : `'success'`, `'error'`, `'warning'`, `'info'`. Défaut : `'success'`.
   *
   * @example
   * this.toastService.show('Données sauvegardées avec succès');
   * this.toastService.show('Erreur de validation', 'error');
   */
  show(message: string, type: ToastType = 'success'): void {
    ...
    this.renderer.addClass(messageText, "text-white")
    ...
    this.renderer.addClass(closeButton, "text-gray-300")
  ...
  /**
   * Crée un SVG représentant l'icône correspondant au type de toast.
   *
   * @param type - Le type de toast (`success`, `error`, etc.).
   * @returns SVGElement prêt à être inséré dans le DOM.
   */
  private createIcon(type: ToastType): SVGElement {
  ...
  /**
   * Crée un SVG représentant une icône de fermeture (croix).
   *
   * @returns SVGElement prêt à être inséré dans un bouton.
   */
  ...
  /**
   * Supprime une notification du DOM avec animation.
   *
   * @param notification - Élément de notification à retirer.
   */
